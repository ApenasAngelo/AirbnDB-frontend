import type { Listing, Property, Host, NeighborhoodStats, HeatmapPoint } from '../types';
...
const firstNames = [
  ...
  'Rafael', 'Camila', 'Fernando', 'Patricia', 'Gustavo', 'Mariana', 'Ricardo',
];
...
const generateMockProperties = (count: number): Property[] => {
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      id: `property-${i + 1}`,
      name: `${type === 'apartment' ? 'Apartamento' : type === 'house' ? 'Casa' : type === 'room' ? 'Quarto' : 'Acomodação'} em ${neighborhood.name}`,
      description: `Linda ${type === 'apartment' ? 'apartamento' : type === 'house' ? 'casa' : type === 'room' ? 'quarto' : 'acomodação'} localizada no coração de ${neighborhood.name}. Perfeita para quem busca conforto e praticidade. Próximo a restaurantes, praias e pontos turísticos.`,
      type,
      capacity,
      bedrooms,
...
const generateMockListings = (properties: Property[], hosts: Host[]): Listing[] => {
  return properties.map((property, i) => {
    ...
    if (['Leblon', 'Ipanema'].includes(property.neighborhood)) {
      basePrice = 300;
    } else if (['Copacabana', 'Barra da Tijuca'].includes(property.neighborhood)) {
      basePrice = 250;
    } else if (['Botafogo', 'Lagoa', 'Flamengo'].includes(property.neighborhood)) {
      basePrice = 200;
    }
...
export const api = {
  ...
  getNeighborhoodStats: async (): Promise<NeighborhoodStats[]> => {
    ...
    const statsByNeighborhood = new Map<string, {
      totalPrice: number;
      totalRating: number;
      count: number;
      ratedCount: number;
    ...
    return Array.from(statsByNeighborhood.entries()).map(([neighborhood, stats]) => ({
      neighborhood,
      averagePrice: Math.round(stats.totalPrice / stats.count),
      averageRating: stats.ratedCount > 0 
